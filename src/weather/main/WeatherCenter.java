...
public class WeatherCenter {
    ...
    public void refresh() {
        ...
        for (WeatherData d : updates) {
            ...
            System.out.printf("→ Temperature: %.1f°C (%.1f°F), feels like: %.1f°C%n", d.temperatureC, toF(d.temperatureC), feels);
        ...
        System.out.println(" End of " + strategy.getName() + " updates.\n");
    }
    ...
    private static double feelsLikeC(double tC, double humidity, double windSpeed) {
        if (tC >= 27.0 && humidity >= 40.0) {
            return heatIndexC(tC, humidity);//hot
        }
        if (tC <= 10.0 && windSpeed >= 5.0) {
            return windChillC(tC, windSpeed);//it's like cold/wind
        }
        return tC;
    }
